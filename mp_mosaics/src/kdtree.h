...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode* maketree(vector<Point<Dim>>& points, int dimension, int left, int right);
//typename KDTree<Dim>::KDTreeNode * KDTree<Dim>::maketree()
    ...
    Point<Dim> nearesthelper(const Point<Dim>& query, KDTreeNode* node, int depth) const;
